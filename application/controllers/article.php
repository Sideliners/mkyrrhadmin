...
class Article extends MY_Controller{
	...
	public function create_enterprise(){
        if($this->user->is_logged_in()){
			$contentdata['script'] = array('admin', 'tinymce/tinymce.min', 'articles');
			$contentdata['styles'] = NULL;
			
			$enterprise_id = $this->uri->segment(3);

            if(!is_numeric($enterprise_id)) redirect(site_url('404_override'));

            $enterprise = $this->mod_enterprise->get_enterprise($enterprise_id);

			$pagedata = $this->_page_defaults('Article for '.$enterprise->enterprise_name, 'prod_articles', 'newarticle');
            ...
            if(isset($_POST['save_article'])){
                if($this->form_validation->run('create_article') == FALSE){
					...
					if($upload->status){
						...
						$article_id = $this->mod_article->create_article($article_data, $enterprise_id);
    ...
    public function update(){
        if($this->user->is_logged_in()){
            ...
            if(!is_numeric($aid) && !is_numeric($id) && !is_string($content)) redirect(site_url('404_override'));
                ...
                if(is_object($article)){
                    ...
                    if(isset($_POST['save_article'])){
                        if($this->form_validation->run('create_article') == FALSE){
                            ...
                            if($_FILES['article_image']['error'] == 0){
                                $upload = (object)$this->upload_image($id);
